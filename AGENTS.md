...
先阅读 README.md、package.json 和源码公开声明。当前已实现摄像头、图片和存储线路；不要把未实施、未构建或未真机验证的内容报告为已支持。
...
- 日志使用全局 business/performance 位掩码、原生 OSLog/Logcat 输出；禁止把原始响应、凭据、提示词或原生异常正文写入日志。统计回调只读取当前平台支持的属性；不恢复临时逐帧日志。
- Git 提交使用英文 Conventional Commits，按改动范围完成必要检查，不提交本地配置或密钥。分支约定：使用 main / develop / feature/<开发者>-v<版本号>，当前开发分支为 feature/yueting-v1.0.0。
