...
实时页面让自绘返回按钮、系统返回与侧滑返回直接完成原生导航，不使用 usePreventRemove 拦截后重新派发 action。之前等待 manager.close 再返回的实现会使 iOS 先恢复被拦截的页面，再执行第二次退出，出现侧滑后回弹。现在页面卸载时先标记操作失效、移除监听，再异步 close；释放不阻塞导航，迟到结果不能更新已卸载页面。侧滑中途取消不卸载页面，也不提前停止预览。进入功能页后不从该页继续 push 新媒体页面。
