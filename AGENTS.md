...
- Android 图片使用原生固定尺寸帧，绕过已复现裁切的 dummy capture；RN iOS 暂保留厂商 setDummyCaptureImagePath，本地均用 RN Image 预览。不主动发送输入 SEI，保留接收任务确认。没有插帧、本地视频生成、Web、Expo Go、逐帧 JS 输出或录制入口。
